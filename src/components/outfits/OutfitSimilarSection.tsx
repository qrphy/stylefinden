...
import type { RelatedByPieceOutfit } from "@/components/outfits/OutfitDetail"
...
export default function OutfitSimilarSection({ outfitsByPieces }: Props) {
  if (outfitsByPieces.length === 0) return null

  const allRelatedPieces = outfitsByPieces.flatMap((o) => o.pieces ?? [])
  const similarPieceGroups = PIECE_GROUPS
    .map((g) => ({
      ...g,
      pieces: allRelatedPieces.filter(
        (p, i, arr) =>
          g.types.includes(p.type ?? "") &&
          p.image &&
