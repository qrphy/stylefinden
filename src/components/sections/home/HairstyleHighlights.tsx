...
export default async function HairstyleHighlights() {
  const hairstyles: HairstyleItem[] = await client.fetch(HOME_HAIRSTYLES_QUERY, {}, { next: { revalidate: 3600, tags: ['hairstyle'] } });
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 xl:px-12 py-16 md:py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
          ...
          {featured.map((h) => (
            ...
            >
              <ImgPlaceholder src={h.image ? urlFor(h.image).width(600).height(800).url() : undefined} alt={h.title} />
          ...
          {rest.map((h) => (
            <a key={h._id} href={`/hairstyles/${h.slug}`} className="group flex flex-col gap-3">
              <div className="relative overflow-hidden bg-gray-100 aspect-[3/4]">
                <ImgPlaceholder src={h.image ? urlFor(h.image).width(400).height(533).url() : undefined} alt={h.title} />
